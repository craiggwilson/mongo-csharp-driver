...
namespace MongoDB.Driver.Relinq.Preparation
{
    internal class SerializerBuilder
    {
        ...
        {
            if (node is SerializationExpression)
            {
                return ((SerializationExpression)node).Serializer;
            }
        ...
        private class PreviouslyUsedSerializerFinder : MongoExpressionVisitor
        {
            ...
            protected internal override Expression VisitDocument(DocumentExpression expression)
            {
                if (expression.Serializer.ValueType == _valueType)
                {
                    _serializer = expression.Serializer;
                    return expression;
                }

                return base.VisitDocument(expression);
            }
            ...
            protected internal override Expression VisitField(FieldExpression expression)
            {
                ...
                return base.VisitField(expression);
            }
        }
    }
}
